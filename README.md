...
[test_filecat.py](examples/test_filecat.py) for a few demos).
...
- A kettlingar RPC method can delegate responding to another microservice,
  ...
  `call_reply_to_fd=...` as an argument to the autogenerated client helper function (or `RPCKitten.call()` directly).
- A server process can listen for incoming TCP/IP client connections,
